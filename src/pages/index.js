...
var DownloadCount = 0;
function onDownload({nativeEvent}, {dzRef, setPreview, imgSize, scale})
{
...
export default () =>
{
    ...
    const step4Class = sectionClass + (preview ? "" : "text-secondary not-allowed" );

    const step3BtnClass = "btn " + (preview ? "btn-primary" : "btn-light")
    const step4BtnClass = "btn " + (preview ? "btn-success" : "btn-light")
        ...
        <div>
            ...
            <section className={step3Class}>
                <h3 className={titleClass}>
                    3.{' '}
                    <button className={step3BtnClass}
                        onClick={ e=> onDownload(e, {dzRef, setPreview, ratio, ...dzProps})}
                    >
                        Render
                    </button>
                </h3>
            ...
            <section className={step4Class}>
                <h3  className={titleClass}>
                    ...
                    <a role="button" className={step4BtnClass} href={preview} target="_blank" download={'InstaFit-'+Date.now()}>
                        Right Click to Save render image
                    </a>
                </h3>
            </section>
        </div>
